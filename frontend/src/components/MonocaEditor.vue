<template>
    <div class="monaco-editor" ref="monacoEditor" :style="style"></div>
</template>

<script setup lang="ts">
import { watch, ref, onMounted, onBeforeUnmount, computed, toRefs, toRaw } from 'vue';
...
const initEditor = () => {
    if (monacoEditorInstance.value) {
        //实例销毁
        toRaw(monacoEditorInstance.value)!.dispose();
    }

    monacoEditorInstance.value = monaco.editor.create(monacoEditor.value!, {
        ...editorOptions,
        ...props.options,
    });

    // 监听值的变化
    toRaw(monacoEditorInstance.value)!.onDidChangeModelContent(() => {
        const value = toRaw(monacoEditorInstance.value)!.getValue(); //给父组件实时返回最新文本
...
watch(
    ...
    (newValue) => {
        if (monacoEditorInstance.value) {
            toRaw(monacoEditorInstance.value)!.updateOptions(newValue);
        }
    },
    { deep: true }
);
...
watch(
    ...
    (newValue) => {
        if (monacoEditorInstance.value) {
            monaco.editor.setModelLanguage(toRaw(monacoEditorInstance.value)!.getModel()!, newValue);
        }
    }
);
